jQuery(function($){  
  ...
  $('.tabs1__caption').on('click', '.caption-item:not(.active)', function() {    
    $(this)
      .addClass('active').siblings().removeClass('active')
      .closest('div.tabs1').find('div.tabs1__content').removeClass('active').eq($(this).index()).addClass('active');
  }); 
  ...
  $('.page-template-Cloud-migration-services .services-page-section__link').on('click', function (e) {
    ...
    return {
      init: function (element,  resolution) {
        this.event(element,  resolution)
      },

      event: function (element,  resolution) {
